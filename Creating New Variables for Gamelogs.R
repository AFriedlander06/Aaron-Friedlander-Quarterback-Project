...
setwd("C:/Users/Aaron Friedlander/Desktop/Thesis Interpretings csvs")
write.csv(basefile, "DatawithWinProb.csv")
...
### Learning
testdf$PtsonDrv <- ""
testdf <- testdf %>% select(-(X.2:X))
curpos <- testdf[4,]$Possession
numlist <- c(5:length(testdf$Possession))
if(isTRUE(curpos==colnames(testdf)[8])){
  tminput <- testdf[8]
  store <- colnames(testdf)[9]
}
if(isTRUE(curpos==colnames(testdf)[9])){
  tminput <- testdf[9]
  store <- colnames(testdf)[8]
}
tmscore <- tminput[4,]
for(num in numlist){
  if(isTRUE(testdf$Possession[num]==testdf$Possession[num+1])){
    teststr <- grepl("onside", testdf$Detail[num])
    if(isTRUE(teststr)){
      if(isTRUE(testdf$Possession[num+1]==curpos)){
        testdf$PtsonDrv[num] = as.numeric(tminput[num,])-as.numeric(tmscore)
        if(isTRUE(curpos==colnames(testdf)[8])){
          tminput <- testdf[8]
          store <- colnames(testdf)[9]
        }
        ...
        tmscore <- tminput[(num+1),]
      }
      
    }
    teststr1 <- grepl("kicks", testdf$Detail[num]) & grepl("fumbles", testdf$Detail[num])
    if(isTRUE(teststr1)){
      if(isTRUE(testdf$Possession[num+1]==curpos)){
        testdf$PtsonDrv[num] = as.numeric(tminput[num,])-as.numeric(tmscore)
        if(isTRUE(curpos==colnames(testdf)[8])){
    ...
    teststr2 <- grepl("punts", testdf$Detail[num]) & grepl("fumbles", testdf$Detail[num])
    if(isTRUE(teststr2)){
      if(isTRUE(testdf$Possession[num+1]==curpos)){
        testdf$PtsonDrv[num] = as.numeric(tminput[num,])-as.numeric(tmscore)
        if(isTRUE(curpos==colnames(testdf)[8])){
  ...
  if(isFALSE(testdf$Possession[num+1]=="")){
    if(isFALSE(testdf$Possession[num]==testdf$Possession[num+1])){
      testdf$PtsonDrv[num] = as.numeric(tminput[num,])-as.numeric(tmscore)
      dummy <- curpos
      curpos <- store
  ...
  if(isTRUE(testdf$Possession[num+1]=="")){
    testdf$PtsonDrv[num] = as.numeric(tminput[num,])-as.numeric(tmscore)
  }
}
...
nonemptytest <- testdf1$PtsonDrv[testdf1$PtsonDrv!=""]
numlist <- c(4:length(testdf1$Possession))
varkey <- 1
for(num in numlist){
  if(isTRUE(testdf1$PtsonDrv[num]=="")){
    testdf1$PtsonDrv[num] = nonemptytest[varkey]
  }
  else{
    varkey <- varkey + 1
  }
